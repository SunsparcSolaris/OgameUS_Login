...
$confnewpass = md5(mysql_real_escape_string($_POST['confnewpass']));

/*Select info from the database. usr must match session user and pass must match the provided oldpass*/
$row = mysql_fetch_array(mysql_query("SELECT pass FROM login WHERE usr='{$_SESSION['usr']}' and pass='{$oldpass}'"));

if ( $oldpass == $row['pass'] && $newpass == $confnewpass && count($row) > 0 ) {
mysql_query("UPDATE login SET pass = '{$newpass}' WHERE usr = '{$_SESSION['usr']}'");
echo " <center>Password Updated. You may return to the Portal Page.</center> ";
}
elseif ( $oldpass !== $row['pass'] && $_POST['chgpassbutton'] == "Change Password" ) {
